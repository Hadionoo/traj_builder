...
void TrajBuilder::build_braking_traj(nav_msgs::Odometry current_vel, geometry_msgs::PoseStamped start_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
 	nav_msgs::Odometry des_state;
    des_state.header = start_pose.header; //really, want to copy the frame_id
    des_state.pose.pose = start_pose.pose; //start from here
    double x_start = start_pose.pose.position.x;
    double y_start = start_pose.pose.position.y;
    double speed_current = current_vel.twist.twist.linear.x;
    double spin_current = current_vel.twist.twist.angular.z;
    ROS_INFO("Braking");
    double psi_start = convertPlanarQuat2Psi(start_pose.pose.orientation);
    ...
    while(speed_current > 0){
    	t +=dt_;
        speed_des = speed_current - accel_max_*t;
        des_state.twist.twist.linear.x = speed_des; //update speed
        //update positions
        x_start = x_start - 0.5 * accel_max_ * t * t * cos(psi_start);
        y_start = y_start - 0.5 * accel_max_ * t * t * sin(psi_start);
        speed_current = speed_des;
